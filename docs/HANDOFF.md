...
| 탭 | 화면 | 실제 동작 여부 |
|---|---|---|
| 🏰 요새 | `IdleScreen` | ✅ 방치 자동전투 · 난이도 4단 · 배속×2 · ⏸ |
| 🌄 필드 | `FieldScreen` | 월드맵 노드 9개(전부 준비 중 패널로 진입) |
| 🏛️ 길드 | `GuildScreen` | 골격만(검색·랭킹·창설·일괄신청 → 준비 중 패널) |
| 🦸 영웅 | `HeroScreen` → `HeroDetail` · `FormationModal` | ✅ 카드 그리드·속성 필터·상세·편성 프리셋 |
| 🎁 혜택 | `PerkScreen` | ✅ **일일 출석만 실제 동작**(7일 순환), 나머지 서브탭은 준비 중 |
| ⚔️ 모험 | `AdventureScreen` | ✅ 캠페인 12챕터 보스전·정주행 |
...
### 🔴 Gim 결정 대기
1. **명세 7번 — 편성에서 `전투` 눌러 시작.** 그대로 옮기면 방치형이 아니게 된다. 명세도 "판단 필요"로 빼둔 항목.
2. **필드/길드/혜택 노드 안을 채울 것인가** — 채우려면 파킹 모듈을 되살려야 한다(`tower`·`arena`·`expedition`·`guild`·`events`·`season`·`shop`).
3. **App.js(419줄)·useGame.js(357줄) 분리** — 규칙 14 대상인데 이 작업 자체가 규칙 13(승인 게이트)에 걸린다.

### 🔧 경로가 끊긴 코어 기능 (우선순위)
| 순위 | 기능 | 영향 |
|---|---|---|
| **1** | **소환** | **영웅이 1명에서 안 늘어난다.** 편성·진형·속성 필터·시너지가 전부 빈 화면이라 확인이 안 된다 |
| 2 | 환생 | `accountMods.powerMult`가 ×1 고정 |
| 3 | 도감·업적·시즌 | 리텐션 축이 통째로 없음 |
| 4 | 본진 | 코어만 살아 있음 |
...
- 테스트 **299개** — `node --test system/test/*.test.mjs`. **완료 보고 전에 반드시 돌린다.**
- 브라우저 페인은 컴포지팅이 멈춰 **스크린샷·터치 조작이 불가**하다. 렌더 텍스트 확인까지만 신뢰하고, 조작 확인은 Gim 폰에 맡긴다.
