...
| 탭 | 화면 | 실제 동작 여부 |
|---|---|---|
| 🏰 요새 | `IdleScreen` → `SummonScreen` | ✅ 방치 자동전투 · 난이도 4단 · 배속×2 · ⏸ · **🗿영웅 제단(모집)** |
...
| ⚔️ 모험 | `AdventureScreen` + `PartyStrip` | ✅ 캠페인 12챕터 보스전·정주행 · **하단 편성 줄(파티 편입 유일 경로)** |
...
요새 탭 우하단 **🗿 영웅 제단** → `SummonScreen`. 호드워 실기 캡처 2장 기준으로 새로 만들었다.
...
### 🔴 Gim 결정 대기
1. ~~**명세 7번**~~ — **해결됨.** 실기 캡처 재확인 결과 호드워도 **상시 자동 전투**였다(모험 탭에 `자동 전투 상태` 타이머가 돌고 있다). 전제가 틀렸던 것. 상세는 `docs/HORDWAR_SPEC.md` "7번 전제 정정".
   - 남은 일: **편성 화면에 `전투`(닫고 전투로 복귀) 버튼 추가** — 호드워와 같아지려면 이것만 하면 된다. 난이도 낮음.
2. **필드/길드/혜택 노드 안을 채울 것인가** — Gim 결정(2026-07-26): **나중에.** 먼저 소환으로 영웅을 늘려 편성·진형·시너지 UI를 검증한다.
3. **App.js(412줄)·useGame.js(357줄) 분리** — 규칙 14 대상인데 이 작업 자체가 규칙 13(승인 게이트)에 걸린다.
...
- 테스트 **306개** — `node --test system/test/*.test.mjs`. **완료 보고 전에 반드시 돌린다.**
