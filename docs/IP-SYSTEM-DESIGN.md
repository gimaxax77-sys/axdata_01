...
**유물**(`core/relics.mjs`) — 계정 단위 영구 배수(전투력/수입). 환생과 함께
`accountMods(state)`로 합산되어 전투(`resolve` powerMult)와 방치 수입(`idle.tick`
currencyMult/growthMult)에 곱해진다. currency로 강화하는 장기 성장 싱크.

> `accountMods`가 **환생 + 유물**을 한 곳에서 합쳐, 새 계정 성장 축(펫 등)을 붙일 때도
> 여기 한 곳만 확장하면 전투·수입에 자동 반영된다.

앱: `콘텐츠` 탭(출석·미션·던전·유물). 하위호환 — 구버전 세이브는 `normalize`가 기본값 주입.
