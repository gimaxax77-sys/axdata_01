...
## 테스트 모드 — 인앱 조건·제약 전면 해제 (2026-07-27)

**Gim 지시**: "현재는 테스트 버전이니 인앱내 모든 조건 및 제약 해제해."

스위치는 **`system/core/testmode.mjs` 의 `const ON = true` 한 줄**이다.
**출시 직전에 이 한 줄만 `false`로 바꾸면 전부 원복된다.** 다른 파일은 손대지 않아도 된다.

| 코드 | 무엇 | 원래 조건 | 뚫은 자리 |
|---|---|---|---|
| A | 난이도 해금 | 험난30·지옥60·나락100층 | `difficulty.mjs` `difficultyUnlocked()` |
| B | 콘텐츠 해금 | 소환8·던전20/35·탑40·펫45·문장50·아레나55·정령60·길드75층 | `unlocks.mjs` `isUnlocked()` |
| C | 레벨 상한 | 랭크 × 20 | `units.mjs` `levelCap()` → `TEST_LEVEL_CAP`(999) |
| D | 돌파 조건 | 소환석 또는 동일영웅 중복 소모 | **F로 자동 해결** — 별도 수정 없음 |
| E | 캠페인 챕터 | 이전 챕터 클리어 필요 | `campaign.mjs` `campaignChapters()`·`fightChapter()` |
| F | 재화 소모 | 잔액 부족 시 실패 | `economy.mjs` `spend()` |
| G | 스킬 슬롯 | 랭크+1 (최대 3) | `skills.mjs` `skillSlots()` |

> **왜 `spend()` 한 곳이 D까지 푸는가** — 재화 소모는 소환·레벨업·돌파·스킬·강화가 **전부 이 함수를 지난다.**
> 호출부마다 가드를 넣는 것보다 통로 하나를 뚫는 게 diff도 작고 빠뜨릴 곳도 없다.

### ⚠️ 자동화 테스트에서는 강제로 꺼진다
`node --test`는 자식 프로세스에 `NODE_TEST_CONTEXT`를 넣는다. `testmode.mjs`가 이를 감지해 스스로 꺼진다.
**이 안전장치가 없으면 재화 차감·해금을 단언하는 테스트 300여 개가 조용히 통과만 하고 아무것도 검증하지 못한다.**
`system/test/testmode.test.mjs`가 이 안전장치와 실제 해제 동작을 둘 다 검증한다(자식 프로세스로 앱 조건을 재현).

### 해제하지 않은 것
- **캠페인 클리어 기록(`campaign.cleared`)은 순서대로만 오른다.** 12챕터에 바로 들어가 이길 수는 있지만,
  진행도가 건너뛰어 기록되지는 않는다(세이브가 앞뒤 안 맞는 상태가 되는 걸 막기 위해서다).
- **소환 화면의 `종족 모집`·`우정 모집`** — 제약이 아니라 **구현이 없는 것**이다. 열어도 나올 내용이 없다.
- **🔒 표시 자체** — 어디가 원래 잠기는 자리인지 보여야 출시 때 되돌릴 곳을 찾는다.
