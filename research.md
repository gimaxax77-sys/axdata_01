...
# 2026-07-27 (오후) — 탭 구조 개편 · 전투 연출 전면 손질

## 1. 탭 구조 개편 (Gim 실기 캡처 3장 기준)

Gim 지시: "요새탭 첫번째 이미지처럼 · 모험탭 두번째 이미지처럼 · 전투 시작 누르면 세번째 이미지처럼.
마찬가지로 **기존 모듈 연계 무시하고 요청사항을 최우선으로**."

| 탭 | 전 | 후 |
|---|---|---|
| 요새 | 전투 화면 | **요새 맵**(`FortressScreen` 신규) — 건물 노드 8개 |
| 모험 | 캠페인 챕터 목록 | **스테이지 화면**(`AdventureScreen` 재작성) |
| — | — | `전투 시작` → **전투 화면**(`IdleScreen` → `BattleScreen` 개명 후 이동) |

### "기존 모듈 연계 무시"를 기능 삭제로 읽지 않았다
지시대로만 하면 동작하던 것이 사라진다. 자리를 옮겨 살렸고 답변에 명시했다.
- **캠페인 12챕터** → 모험 탭 상단 **마일스톤 진행바**(캡처의 `7-4·7-8·7-12` 자리). 아이콘 탭 = 보스 도전.
- **난이도 4단** → 모험 스테이지 화면. 전투 화면에서 빼면 고를 경로가 사라진다.
- **캡처의 과금·이벤트 아이콘 20여 개는 만들지 않았다** — 대응 기능이 없어 누르면 아무 일도 안 일어난다.

## 2. 전투 연출 — 지적 5회에 걸친 수정

| 지적 | 원인 · 처리 |
|---|---|
| 적이 3~7마리로 흔들림 | `rollFoes`가 열마다 1~3마리를 굴렸다 → `FOE_COLS`로 **5마리 고정** |
| 적이 전열3·후열2 | **내가 열 순서를 반대로 읽었다.** `sideFoe`가 `row-reverse`라 배열 `[0]`이 오른쪽(후열)이다. `[2,3]`→`[3,2]` |
| 양팀 전투력이 왼쪽에 몰림 | `stageProg`에 `flex:1`이 빠져 폭이 0으로 붕괴 → 두 숫자가 붙어 있었다 |
| 전투력이 위 패널과 안 맞음 | 양 끝에 **보이지 않는 ★**(opacity 0)을 둬 VS 바와 폭을 맞췄다. 여백을 숫자로 박으면 폰트가 바뀔 때 조용히 어긋난다 |
| 데미지 이펙트가 1개뿐 | 💥가 적 한 마리에만 있었고 아군엔 없었다 → 양쪽 5명 전원 |
| 숫자가 누가 맞았는지 모름 | 공용 5칸 슬롯 → **유닛별 `DmgFloat`**. `처치!`만 공용 슬롯에 남겼다 |
| 일시일괄로 터져 생동감 없음 | `useStaggeredPlay(token, idx, duration, step)` — 순번대로 파도처럼. step은 **배속에 비례**(`FX_STEP_MS/speed`) |
| 전체 속도가 빠름 | 연출만 늦춤(Gim 선택) — 틱 150→220ms, `WAVE_DMG` 하향. 웨이브 유지 1.6초→3.9초 |

### 작업 중 잡은 실제 버그
💥의 `Animated.Value` 초기값이 0인데 opacity 보간이 `0→불투명`이라 **첫 공격 전부터 💥가 떠 있었다.**
유닛 1개일 땐 안 보였지만 10개로 늘리면 드러난다. 초기값을 1(연출 끝)로 바꾸고 ⏸ 진입 시에도 되돌린다.

### 속도 실측 (Gim 질문 "배속이 몇인지")
- 배속 버튼은 `×1↔×2` 토글, **기본 ×1**
- 연출: 틱 220ms · 아군 공격 4틱(880ms) · 적 반격 6틱(1320ms)
- **진행 속도는 별개** — `useGame` `TICK_GAME_SEC=24`(실제 1초 = 게임 24초),
  `idle.mjs` `AUTO_ADVANCE_MARGIN=2.5초` → 실제 1초에 최대 9~10층 전진.
  Gim이 "연출만"을 골라 **진행 속도는 손대지 않았다.** 밸런스 볼 때 다시 꺼낼 항목.

## 3. 레이아웃 — 같은 실수 2회

`{cond ? <X/> : null}` 로 그리면 없을 때 높이가 0이라, 나타나는 순간 레이아웃이 밀린다.
형제에 `flex:1`이 있으면 그쪽이 줄며 **위쪽 요소까지** 움직인다.
- `최대 레벨 상승` 버튼 → 자리를 항상 잡고 `opacity:0` + `pointerEvents:none`
- 안내 문구 → 항상 렌더하고 비면 공백 한 칸. **실제로 밀리던 3곳만** 고쳤다
  (`PartyStrip`·`SummonScreen`·`HeroScreen`). `AdventureScreen`·`HeroDetail`은 확인 결과 안 밀린다.

## 4. 그 밖

- 영웅 상세 **서브탭 4종**을 캡처 기준으로 만들고 **각각 모듈로 분리**(규칙 14) —
  `HeroGearPanel`·`HeroAscendPanel`·`HeroCostumePanel`. **초월 = 돌파(ascend)** 가 여기로 왔다.
- 영웅 상세 **좌우 전환 화살표** — 목록으로 나가지 않고 순환식으로 넘긴다. `n/N` 표시.
- 필드 탭 9개 노드를 **스크롤 없이 한 화면**에(`center` + `gap:6`).
- `units.mjs`에 `rankLevelCap(rank)` 추가 — 초월 패널이 `랭크×20` 공식을 복제하지 않게.

## 5. ⚠️ 확인하지 못한 것

**이번 세션에서 화면을 직접 열어 본 것은 요새 탭(기본 탭) 하나뿐이다.**
RN-Web이 합성 클릭을 받지 않아 탭 전환조차 불가능하다. 나머지는 전부
**빌드 포함 여부 + 부팅 콘솔 무에러**까지만 확인했고, 실제 조작·레이아웃은 Gim 폰 제보에 의존했다.
- 서브탭 3종(장비·초월·코스튬) 패널을 한 번도 못 봤다 — 32% 높이 안에 들어가는지 미검증
- 초월(ascend) 실행 후 스킬 슬롯·레벨 상한 반영 미검증
- 레벨 999까지 올렸을 때 전투력 표시·오버플로 미검증
- 필드 탭 간격이 실제로 자연스러운지 미검증

## 6. 사용량 (2026-07-27 16:18 `/usage`)

세션 31%(18:00 리셋) · 주간 45%(7/31 14:00 리셋) · Fable 0%.
...
# 2026-07-29 (오전) — 데미지 숫자 줄바꿈 수정

**Gim 실기 제보**: "적군 데미지 4자리 넘어가면 줄바꿈됨"(캡처 첨부).

**원인** — `BattleView.js`의 `s.uFloat`가 `position:'absolute'`인데 **`left`/`right`가 없었다.**
그러면 폭이 부모(`s.unit`, 유닛 칸 ≈30px)에 묶인다. 3자리(`643`)는 들어가지만
쉼표가 붙는 4자리(`1,234`)부터 넘쳐 두 줄이 됐다.

**수정** — `left:-30, right:-30, textAlign:'center'` 추가(한 줄). 폭 ≈90px 확보.
`DmgFloat`는 **아군(`Ally`)과 적(`s.unit`) 양쪽이 같은 스타일을 쓰므로** 한 곳만 고치면 둘 다 해결된다.
부수 효과로 숫자가 유닛 **왼쪽 치우침 → 머리 위 가운데**로 옮겨졌다(의도한 개선).

**상한 실측** — 표시값은 `enemyDmg(≤0.18) × mul(≤1.9) × 4200 × (0.85~1.15)` = **최대 약 1,652**.
5글자가 최대이므로 90px면 충분하고 다시 터질 여지는 없다. (이 수치는 연출용 고정 공식이고 실제 전투 계산과 별개다.)

**검증** — 테스트 309개 통과 · 번들 해시 변경(`AppEntry-6c225ecb…`) · `play.html`과 배포본 **sha256 일치**(`d1304493…`).
⚠️ **실제 4자리 데미지가 한 줄로 나오는 화면은 보지 못했다** — RN-Web이 합성 클릭을 안 받아 전투 화면에 진입할 수 없다. Gim 폰 확인 필요.

---

# 2026-07-29 (오후) — 뒤로가기 하단 통일 · 하단 탭 초기화

**Gim 지시**: "요새 탭·필드 탭 뒤로가기 버튼이 상단에 있음. 모든 컨텐츠 뒤로가기는 하단으로 전부 통일 배치.
그리고 진입한 위치 상관없이 하단 탭 누르면 해당 탭으로 이동 되도록."

## 1. 뒤로가기 위치 — 고칠 곳은 **한 파일뿐**이었다

먼저 뒤로가기가 있는 자리를 전부 찾았다(`onBack|◀|뒤로` 검색).

| 화면 | 위치 | 처리 |
|---|---|---|
| `ComingSoon.js` | **좌상단** (`position:absolute, left:8, top:8`) | ← **이것만 어긋나 있었다.** 하단으로 이동 |
| `SummonScreen.js` | 하단 좌 붉은 ◀ | 이미 맞음 |
| `HeroDetail.js` | 하단 바 좌 ◀ | 이미 맞음 |
| `BattleScreen.js` | 최하단 좌 붉은 ◀ | 이미 맞음 |

**요새·필드·길드·영웅 상세의 준비 중 패널이 전부 `ComingSoon` 하나를 지난다.**
그래서 파일 하나만 고치면 Gim이 지적한 두 탭을 포함해 전부 통일된다.
모양도 새로 만들지 않고 `SummonScreen`·`BattleScreen`의 붉은 버튼(`#7a2f22`/`#b8543c`/`◀`)을 그대로 썼다 —
호드워 캡처의 "최하단 좌측 붉은 뒤로가기 화살표"와 같다.

## 2. 하단 탭이 같은 탭에선 안 먹던 이유

하위 화면(요새→영웅 제단, 모험→전투, 준비 중 패널)은 **각 화면이 자기 `useState`로** 들고 있다.
다른 탭으로 가면 컴포넌트 자체가 바뀌어 언마운트되므로 초기화되지만,
**같은 탭을 다시 누르면 `setTab`이 같은 값이라 리렌더조차 없어** 아무 일도 일어나지 않았다.

→ `App.js`에 `tabNonce`를 두고 탭을 누를 때마다 +1, `<BaseScreen key={`${tab}:${tabNonce}`}>`로 **강제 재마운트**.
화면 코드는 한 줄도 건드리지 않는다. 하위 상태가 어디에 몇 개 있든 전부 초기화된다.

**자문자답 3가지**
1. `React.memo` 성능 최적화(방치 틱 리렌더 방지)가 깨지지 않나? → **아니다.** key는 탭을 누를 때만 바뀐다. 그 사이엔 그대로다.
2. 전투 중에 탭을 누르면 진행이 날아가나? → **아니다.** 진행은 `useGame`에 있고 화면은 표시만 한다. 상시 자동 전투는 화면과 무관하게 돈다.
3. 마운트될 때 세이브를 건드리는 화면이 있나(중복 수령 위험)? → **없다.** 6개 화면에서 `useEffect`는 `AdventureScreen`의 초 표시 타이머 하나뿐이고, 리셋돼도 무해하다.

**부수 효과** — 같은 탭을 다시 누르면 그 화면의 속성 필터·스크롤 위치·`자동 전투 상태` 타이머도 초기화된다.
"그 탭의 첫 화면으로 간다"는 지시의 당연한 결과지만 눈에 띌 수 있어 적어 둔다.

## 3. 검증

테스트 **309개 통과** · 번들 해시 `6c225ecb…` → `777e2010…` 변경 ·
번들 내 붉은 뒤로 버튼 색(`7a2f22`) **2곳 → 3곳**(ComingSoon 합류 확인) · 배포본 sha256 대조.

⚠️ **확인하지 못한 것** — RN-Web이 합성 클릭을 안 받아 **실제로 눌러 본 것은 없다.**
준비 중 패널이 어떻게 보이는지, 탭 재진입이 실제로 첫 화면으로 가는지는 **Gim 폰 확인 필요**.

---

# 2026-07-29 (밤) — 옛 APK로 확인한 지적 · 좌우 이동 화살표 · APK 빌드 2차 함정

## 1. Gim 지적 2건은 **옛 코드**를 본 것이었다

Gim이 캡처 4장과 함께 "뒤로가기 여전히 상단 · 요새탭 눌러도 안 돌아감"을 제보했다.
**둘 다 이미 고쳐 배포한 것이라, 먼저 어느 빌드를 보고 있는지 확인했다.**
...
**APK가 수정보다 1시간 이르다.** 캡처의 뒤로 버튼이 파란색(옛 `T.surface2`)인 것도 일치한다.
배포된 웹 번들에는 옛 상단 스타일이 **0개**, 새 붉은 하단 버튼이 **3개**로 정상 반영돼 있었다.

> **내 잘못이다.** APK를 만들어 준 뒤에 코드를 고쳤으면서 **"새 APK가 필요하다"고 말하지 않았다.**
> 앞으로 APK 전달 후 코드를 고치면 그 사실을 반드시 함께 알린다.

### 검증하다 낸 오탐 2건 (둘 다 내 방법 문제)
1. `strings` 명령이 이 환경에 **없다.** `strings … | grep -c` 가 조용히 `0`을 내놓아
   "APK에 수정이 없다"는 결론을 낼 뻔했다. → **대조군을 먼저 넣어 방법 자체를 검증**해 걸러냈다.
2. Hermes 번들은 한글을 **UTF-16LE**로 담고 숫자는 문자열로 담지 않는다.
   `grep -a "이전 영웅"`(UTF-8)은 있어도 0이 나온다. 판정은 **UTF-16LE 바이트 비교**로 해야 한다.
   실제 판정 결과 — 대조군 `이전 영웅` 양쪽 1 · 신규 `이전/다음 콘텐츠` **새 APK만 1**.

## 2. 좌우 이동 화살표 (신규 요청)

Gim: "각 탭에서도 영웅탭처럼 컨텐츠 좌우 이동 버튼 추가."

`ComingSoon`에 `onStep(±1)`·`stepInfo` 를 추가하고 세 화면이 목록만 넘긴다 —
요새 **8**(건물) · 필드 **9**(노드) · 길드 **4**(항목), 전부 순환식. 모양·자리는 `HeroDetail`을 그대로 따랐다.

- **혜택 탭은 제외했다.** 진입/뒤로 구조가 아니라 하단 서브탭으로 그 자리에서 바뀌는 화면이라 붙일 자리가 없다.
- 요새에서 화살표로 **영웅 제단**에 닿으면 실제 모집 화면이 뜬다(그 화면엔 화살표가 없어 뒤로로 나온다).
- 길드는 `Object.keys(PAGES)`를 쓴다 — 순서 목록을 두 벌로 두면 항목이 늘 때 조용히 어긋난다.

## 3. ⚠️ APK 빌드 2차 함정 — 한글 경로가 **네이티브 C++**에서 다시 걸렸다

오전에 통과했던 빌드가 밤에는 실패했다.

```
ninja: error: FindFirstFileExA(d:/.code/…/expo-av/android/src/main/cpp): 지정된 경로를 찾을 수 없습니다
```

`ninja`는 옛 ANSI 파일 API를 써서 한글 경로를 못 연다. **`overridePathCheck` 같은 해제 옵션이 없다.**
오전에 통과한 이유는 **C++ 산출물이 7/19 것 그대로 남아 그 단계를 건너뛰었기** 때문이고,
캐시가 무효화되자 바로 드러났다. AGP 경고문("Windows에서 대개 실패한다")이 정확했다.

### 정션은 실패, `subst`가 답이었다
| 시도 | ninja | Metro | 결과 |
|---|---|---|---|
| 한글 경로 그대로 | ❌ | — | 실패 |
| `mklink /J D:\eldria-build` (정션) | ✅ 통과 | ❌ `Unable to resolve module ../../App` | 실패 |
| **`subst X:`** (가상 드라이브) | ✅ | ✅ | **성공 (7분 3초)** |

**차이의 이유** — 정션은 리파스 포인트라 **Node가 realpath로 원래 한글 경로를 되돌려** 해석한다.
`subst`는 장치 매핑이라 되돌려지지 않아 전 구간이 `X:\…` 로만 보인다.
`subst`는 **재부팅하면 사라진다** — 빌드마다 한 줄 먼저 실행한다.

## 4. 결과

- 웹 배포 `?v=41` — 번들 `b2561b42…`, 화살표 스타일 **2벌**(HeroDetail + ComingSoon) 확인, 배포본 sha256 일치
- APK `엘드리아_20260729_2100_뒤로하단·탭초기화·좌우이동.apk` (87.8MB) — 드라이브 복사 후 sha256 일치
- 테스트 **309개 통과**
- 문서: `docs/BUILD_LOCAL.md` B-2에 `subst` 절차 추가(정션이 왜 안 되는지 포함)

## 5. ⚠️ 확인하지 못한 것

1. **좌우 화살표를 실제로 눌러보지 못했다.** RN-Web이 합성 클릭을 안 받는다. 번들 포함까지만 확인했다.
2. **뒤로가기 하단 이동·탭 초기화가 실기에서 동작하는지 아직 못 봤다** — Gim이 본 것은 옛 APK였으므로 **이번 빌드가 첫 실기 검증**이 된다.
3. 구글 드라이브 `APK` 폴더에 **이전 APK 2개(7/25 엘로그·7/29 10:22 엘드리아)가 보이지 않는다.**
   내 작업은 새 이름 복사뿐이라 삭제될 수 없다. Gim이 폰에서 정리했을 가능성이 높지만 **확인하지 못했다.**

---

# [2026-07-30 01:15~01:45] 상태체크 · APK 재빌드 · 빌드 문서 갱신

> 소급 기록(2026-08-09 작성). 당시 세션이 기록 없이 끝나 뒤늦게 채운다.

## 1. 요청

① `멤 상태체크` → AXdata 전 저장소 현황 점검 ② `apk 빌드` ③ `문서 고쳐주고 커밋 푸시 해`

## 2. 상태체크 결과 (12개 git 저장소)

미커밋 7곳 · 푸시 대기 5곳. 브랜치는 제각각(01=`claude/3d-poc`, 09·07=`claude/*`, 13=`atup`, 14=`master`).
`axdata_14_capcut_agent` 는 **원격 저장소가 없어** 백업이 전무했다(→ 2026-08-09 확인 시 `origin` 붙어 해결됨).
`axdata_05` 의 미추적 20여 폴더(`out*/`·`_*/`)는 렌더 산출물로 정상.

## 3. APK — **빌드했으나 파일은 재생성되지 않았다**

빌드 전 조사에서 **앱 코드가 07-29 20:48 이후 변경 0** 임을 확인했다(미커밋 4건은 전부 문서·`package.json`).
Gim 께 "지금 빌드해도 어젯밤 APK와 동일하다"고 알리고 선택을 받아 **그래도 새로 빌드**로 진행했다.

| 항목 | 결과 |
|---|---|
| 게이트 점검 | 8개 항목 전부 통과(`_TOOLS/build-gate`) |
| 빌드 | `BUILD SUCCESSFUL in 40s` · **726 task 중 37개만 실행** |
| APK 파일 | **재생성되지 않음** — 수정 시각 01:15 그대로, sha256 도 빌드 전 측정값과 동일 |
| JS 번들 | `6c548631…46530e5c` — 07-29 21:00 APK 와 **바이트 단위로 동일** |
| 전달 | `G:\내 드라이브\APK\엘드리아_20260730_0140_재빌드.apk` · 원본·사본 sha256 일치 |

**핵심 발견 2가지** (→ 기억 [[apk-build-verification]] 로 기록)
1. **코드가 그대로면 Gradle 은 APK 를 다시 만들지 않는다.** `BUILD SUCCESSFUL` 이 떠도 파일 시각·해시가 그대로다.
   앱 동일 여부는 APK 전체 해시가 아니라 **`assets/index.android.bundle` 해시**로 판정해야 한다(전체 해시는 서명·타임스탬프로 매번 달라짐).
2. **APK 크기 87.8MB(십진) = 83.8MB(탐색기 MiB)** — 같은 파일(87,844,197 B). 내가 탐색기 표기를 그대로 옮겨 Gim 께 잘못 보고했다가 정정했다.
   PowerShell `/1MB` 는 1024 기준이라 기록값(십진)과 어긋난다.

## 4. 문서·커밋

`docs/BUILD_LOCAL.md` 갱신 — 빌드 시간을 캐시 상태별로 분리(**캐시 있으면 40초 / 지운 뒤 5분 39초**, 726 task 중 각 37·649개 실행) · APK 크기 단위 병기 · 게이트 스크립트(`npm run build:apk:local`) 절차 편입 · up-to-date 함정 추가.

커밋 2개로 나눠 `claude/3d-poc` 에 푸시(`8647674..296f903`). 테스트 **309개 전부 통과** 후 커밋.

## 5. ⚠️ 사고 — 커밋 메시지가 `@` 로 나갔다

Bash 도구에 PowerShell 여기-문자열(`@'…'@`)을 써서 커밋 제목 두 개가 모두 `@` 로 시작했다.
**푸시 전에 발견**해 `git reset --soft HEAD~2` 로 되돌리고 heredoc 으로 재커밋했다 — 파일 변경은 손대지 않았고 원격에 깨진 메시지는 나가지 않았다.
같은 실수가 **07-25 두 번, 07-30 한 번으로 최소 3회 재발**했다(→ [[shell-edit-hazards]] 갱신).

## 6. ⚠️ 확인하지 못한 것

- **폰 실기 확인은 여전히 못 했다.** 전달한 APK 가 07-29 21:00 것과 앱이 동일하므로, 좌우 화살표·뒤로가기 하단 이동의 실기 동작은 이 회차에서도 검증되지 않았다.
- 당시 `_TOOLS/build-gate` 는 어느 저장소에도 없어 `package.json` 이 버전관리 밖 스크립트를 참조했다(→ 같은 날 `axdata_tools` 저장소로 해결, [[axdata-tools-repo]]).
