...
export const BALANCE = {
  ...
  // 적 강화율(1.115) < 보상 증가율(1.14) → 수입이 난이도를 앞질러 벽을 완만하게.
  // 지수 완화라 층이 깊을수록 요구 전투력이 크게 낮아진다(층60 −55%, 층80 −65%).
  // 남는 벽은 "성장 비용의 지수화"라 환생(파워 배수)이 자연스레 이를 넘는다.
  enemyGrowth: 1.115, // 스테이지당 적 강화율 (허들 완화)

  // 스테이지 보상 (수입 곡선) — base·증가율 모두 상향해 성장을 가속.
  rewardBase: { currency: 24, growth: 10 },
  rewardGrowth: 1.14, // 스테이지당 보상 증가율 (난이도보다 빠르게)
