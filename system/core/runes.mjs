...
// 룬 등급 — 메인스탯 배수. 발굴 확률로 결정.
export const RUNE_RARITY = {
  N: { id: 'N', label: '일반', mult: 1.0, weight: 55 },
  R: { id: 'R', label: '고급', mult: 1.5, weight: 32 },
  SR: { id: 'SR', label: '희귀', mult: 2.2, weight: 13 },
};
...
export function summonRune(state, rng = Math.random) {
  ...
  const rarity = pickWeighted(rng, Object.values(RUNE_RARITY), 'weight').id;
  const rune = { uid: `r${++_rseq}`, set, rarity, level: 0 };
