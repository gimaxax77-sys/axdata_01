...
//   R   1.10 × 1.22 = 1.342
//   SR  1.22 × 1.14 = 1.391
//   SSR 1.36 × 1.08 = 1.469   ← 최고등급(=UR) 아래에 머문다 (1.300 < 1.360 무발현 SSR)
//
...
const TIER = { N: 0, R: 1, SR: 2, SSR: 3 };

// 등급 기본 스탯 배수 (등급이 곧 잠재력의 하한). 등급 없으면 1.0(하위호환).
export const RARITY_BASE_MULT = { N: 1.0, R: 1.10, SR: 1.22, SSR: 1.36 };

// 완전 발현(6/6) 시 총 씨앗 보정. 낮은 등급일수록 크다.
export const SEED_FULL = { N: 0.30, R: 0.22, SR: 0.14, SSR: 0.08 };
...
const CONDITIONS = [
  { id: 'talent', label: '재능 각성', narr: '잠든 재능이 깨어난다', stat: 'hp', metric: (u) => u.level, thr: [20, 40, 60, 80], unitLabel: '레벨' },
  { id: 'breakthrough', label: '한계 돌파', narr: '벽을 넘어선 자', stat: 'atk', metric: (u) => u.rank, thr: [2, 3, 4, 5], unitLabel: '랭크' },
  { id: 'bond', label: '유대', narr: '곁을 지키는 마음', stat: 'def', metric: (u) => intimacyLevel(u), thr: [2, 4, 6, 8], unitLabel: '친밀도' },
  { id: 'oath', label: '무기의 서약', narr: '무기와 하나가 되다', stat: 'atk', metric: (u) => (u.sigWeapon?.level || 0), thr: [3, 6, 9, 12], unitLabel: '전용무기' },
  { id: 'resonance', label: '심연의 공명', narr: '내면이 공명한다', stat: 'spd', metric: (u) => (u.sigAwaken || 0), thr: [1, 2, 3, 3], unitLabel: '각성' },
  { id: 'runeway', label: '룬의 인도', narr: '룬이 길을 연다', stat: 'hp', metric: (u) => (u.runes || []).filter(Boolean).length, thr: [1, 2, 3, 3], unitLabel: '룬 장착' },
];
